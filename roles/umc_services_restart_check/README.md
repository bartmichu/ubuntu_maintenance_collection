...
The `umc_services_restart_check` role leverages the `needrestart` utility to determine which services need to be restarted after system changes (like kernel updates or library upgrades).

The role sets a host fact `_umc_global_services_to_restart` with the names of the services.
